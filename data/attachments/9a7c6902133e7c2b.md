...
- Name: ClientAppPO.spec.js >> Client App E2E testing for:ZARA COAT 3
...
```ts
  ...
  2  |     constructor(page) {
  3  |         this.page = page;
  4  |         this.signInButton = page.getByRole("button", { name: 'login' });
  5  |         this.email = page.getByPlaceholder("email@example.com");
  6  |         this.password = page.getByPlaceholder("enter your passsword");
  7  |     }
  8  |     async gotToLoginPage() {
> 9  |         await this.page.goto("https://rahulshettyacademy.com/client/#/auth/login")
     |                         ^ Error: page.goto: Test timeout of 40000ms exceeded.
  10 | 
  11 |     }
  12 |     async validLogin(userName, password) {
  13 |         await this.email.fill(userName);
